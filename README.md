...
`prinstall` collapses the whole workflow into a single 14 MB binary. Scan a subnet, add a printer, remove it cleanly, audit what's installed. Works over SSH or RMM remote shells with a clean CLI (`--json` on every command), or drops into a dense lazy-style TUI when launched from a real terminal.
